...
**EcoCity** es una aplicación móvil diseñada para permitir a los ciudadanos reportar incidencias urbanas en tiempo real, como baches, farolas rotas o acumulación de basura. Los usuarios pueden reportar incidencias mediante un formulario, adjuntar fotos o notas de voz y compartir la ubicación precisa a través del GPS. Además, la aplicación incluye un sistema de autenticación para gestionar el acceso de los usuarios.
...
- **`res/layout/`**:
  - **activity_main.xml**: Layout para la pantalla principal con el **RecyclerView**.
  - **activity_login.xml**: Layout para la pantalla de inicio de sesión.
  - **item_incidencia.xml**: Layout para cada elemento en la lista del **RecyclerView**.

- **`AndroidManifest.xml`**:
  - Configuración de las actividades (`MainActivity` y `LoginActivity`) y los permisos necesarios (por ejemplo, para el acceso a la cámara y a la ubicación).
